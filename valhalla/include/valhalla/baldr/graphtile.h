...
// CARTOHACK
  ...
  // CARTOHACK
  /** Load tile bytes into the internal graphtile byte buffer
   * @param  graphid     the id of the tile to be decompressed
   * @param  tile_data   the uncompressed bytes
   * @return whether or not the graphtile has been successfully initialized with
   *         the uncompressed data
   */
  bool LoadTile(const GraphId& graphid, std::vector<char>& tile_data);
