...
bool GraphTile::LoadTile(const GraphId& graphid, std::vector<char>& tile_data) {
  graphtile_.reset(new std::vector<char>(tile_data));
  Initialize(graphid, graphtile_->data(), graphtile_->size());
  return true;
}
...
GraphTile GraphTile::CacheTileURL(const std::string& tile_url,
  ...
  if (!cache_location.empty()) {
    ...
    auto disk_location = cache_location + "/" + suffix;
    // CARTOHACK
...
GraphId GraphTile::GetTileId(const std::string& fname) {
  ...
  allowed.erase(static_cast<std::string::value_type>('/'));
