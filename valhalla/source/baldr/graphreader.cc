...
private:
  ...
  static GraphId ToGraphId(const std::tuple<int, int, int>& tile_coords) {
    ...
    auto it = levels.find(std::get<0>(tile_coords));
    if (it == levels.end()) {
      return GraphId();
    }
  ...
  static std::tuple<int, int, int> FromGraphId(const GraphId& graphid) {
    const auto& levels = TileHierarchy::levels();
    auto it = levels.find(graphid.level());
    if (it == levels.end()) {
...
TileCache* TileCacheFactory::createTileCache(const boost::property_tree::ptree& pt) {
  ...
  // CARTOHACK
  // Use synchronized cache by default
  if (pt.get<bool>("global_synchronized_cache", true)) {
    if (!globalTileCache_) {
      if (use_lru_cache) {
...
GraphReader::GraphReader(const boost::property_tree::ptree& pt)
    : tile_extract_(get_extract_instance(pt)), tile_dir_(pt.get<std::string>("tile_dir", "")),
      curlers_(stdext::make_unique<curler_pool_t>(pt.get<size_t>("max_concurrent_reader_users", 1),
...
bool GraphReader::DoesTileExist(const GraphId& graphid) const {
  ...
  std::string file_location =
      tile_dir_ + "/" + GraphTile::FileSuffix(graphid.Tile_Base());
  struct stat buffer;
...
const GraphTile* GraphReader::GetGraphTile(const GraphId& graphid) {
  ...
  if (mbtiles_db_) {
    ...
    if (mbtiles_db_->ReadTile(graphid, tile_data)) {
      GraphTile tile;
      tile.LoadTile(graphid, tile_data);
...
bool GraphReader::AreEdgesConnected(const GraphId& edge1, const GraphId& edge2) {
  ...
  if (de1_opp && de2 && (de1_opp->endnode() == de2->endnode() || is_transition(de1_opp->endnode(), de2->endnode()))) {
    return true;
  }
  ...
  // CARTOHACK
  if (de1 && de1_opp && de2_opp && (de2_opp->endnode() == de1->endnode() || de2_opp->endnode() == de1_opp->endnode() ||
