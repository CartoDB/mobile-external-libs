...
void from_json(rapidjson::Document& doc, Options& options) {
  ...
  // CARTOHACK
  doc.AddMember(rapidjson::Value("language", allocator), rapidjson::Value(options.language().c_str(), allocator), allocator);

  std::string formatVal = valhalla::Options_Format_Enum_Name(options.format());
  doc.AddMember(rapidjson::Value("format", allocator),
                rapidjson::Value(formatVal.c_str(), allocator), allocator);
}
