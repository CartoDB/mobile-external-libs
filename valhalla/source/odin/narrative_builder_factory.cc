...
std::unique_ptr<NarrativeBuilder> NarrativeBuilderFactory::Create(const Options& options,
  ...
  if (phrase_dictionary->second->GetLanguageTag() == "cs-CZ") {
    return stdext::make_unique<NarrativeBuilder_csCZ>(options, trip_path, *phrase_dictionary->second);
  } else if (phrase_dictionary->second->GetLanguageTag() == "hi-IN") {
    return stdext::make_unique<NarrativeBuilder_hiIN>(options, trip_path, *phrase_dictionary->second);
  } else if (phrase_dictionary->second->GetLanguageTag() == "it-IT") {
    return stdext::make_unique<NarrativeBuilder_itIT>(options, trip_path, *phrase_dictionary->second);
  } else if (phrase_dictionary->second->GetLanguageTag() == "ru-RU") {
    return stdext::make_unique<NarrativeBuilder_ruRU>(options, trip_path, *phrase_dictionary->second);
  }

  // otherwise just return pointer to NarrativeBuilder
  return stdext::make_unique<NarrativeBuilder>(options, trip_path, *phrase_dictionary->second);
}
