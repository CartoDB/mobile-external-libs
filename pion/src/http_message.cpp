                          ...
                          asio::error_code& ec, bool headers_only)
{
    ...
    if (!headers_only && get_content_length() > 0 && get_content() != NULL)
		write_buffers.push_back(asio::buffer(get_content(), static_cast<std::size_t>(get_content_length())));

    // send the message and return the result
    return tcp_conn.write(write_buffers, ec);
}

message::content_length_t message::receive(tcp::connection& tcp_conn,
                             asio::error_code& ec,
                             parser& http_parser)
{
    ...
    while (true) {
        ...
        if (ec || last_bytes_read == 0) {
            if (http_parser.check_premature_eof(*this)) {
                ...
                if (! ec)
                    ec = make_error_code(asio::error::connection_aborted);
...
message::content_length_t message::receive(tcp::connection& tcp_conn,
                             asio::error_code& ec,
                             bool headers_only,
                             content_length_t max_content_length)
{
...
message::content_length_t message::read(std::istream& in,
                          asio::error_code& ec,
                          parser& http_parser)
{
