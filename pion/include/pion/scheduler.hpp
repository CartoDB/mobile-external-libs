...
public:
    ...
    inline static void sleep(std::uint32_t sleep_sec, std::uint32_t sleep_nsec) {
        std::this_thread::sleep_for(std::chrono::milliseconds(sleep_sec * 1000 + sleep_nsec / 1000));
    }
    ...
    {
        wakeup_condition.wait_for(wakeup_lock, std::chrono::milliseconds(sleep_sec * 1000 + sleep_nsec / 1000));
    }
