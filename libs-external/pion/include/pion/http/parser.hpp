...
public:
	/// content-length type
	typedef std::uint64_t content_length_t;
    ...
    {
    public:
        const char *name() const ASIO_ERROR_CATEGORY_NOEXCEPT { return "parser"; }
     ...
     */
    parser(const bool is_request, content_length_t max_content_length = DEFAULT_CONTENT_MAX)
    ...
    inline content_length_t get_total_bytes_read(void) const { return m_bytes_total_read; }

    /// returns the total number of bytes read while parsing the payload content
    inline content_length_t get_content_bytes_read(void) const { return m_bytes_content_read; }

    /// returns the maximum length for HTTP payload content
    inline content_length_t get_max_content_length(void) const { return m_max_content_length; }
    ...
    inline void set_payload_handler(const payload_handler_t& h) { m_payload_handler = h; }

    /// sets the maximum length for HTTP payload content
    inline void set_max_content_length(content_length_t n) { m_max_content_length = n; }
...
private:
    ...
    content_length_t                    m_bytes_content_remaining;

    /// number of bytes read so far into the message's payload content
    content_length_t                    m_bytes_content_read;
    ...
    content_length_t                    m_bytes_total_read;

    /// maximum length for HTTP payload content
    content_length_t                    m_max_content_length;
